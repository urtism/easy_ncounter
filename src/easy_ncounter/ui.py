...
def _run_app() -> None:
    ...
    with module_stats:
        ...
        if not _metadata_ready():
            ...
            settings_ready = bool(settings) and settings["reference_group"] != settings["case_group"]
            if not settings_ready:
                st.warning("Select two distinct groups for the comparison.")
...
def _render_analysis_settings(metadata: pd.DataFrame) -> dict[str, object]:
    ...
    groups = [str(value) for value in metadata[group_col].dropna().unique()]
    if not groups:
        st.error("The selected group column does not contain usable values.")
        return {}
    if len(groups) < 2:
        st.warning("At least two groups are required for differential comparison.")
    reference_group = st.selectbox("Reference group", groups, index=0)
    case_index = 1 if len(groups) > 1 else 0
    ...
    method = st.selectbox("Normalization", ["nanostringnorm", "library_size"])
...
def _materialize_run(
    ...
    if processed_counts:
        ...
        if edited_samplesheet is not None:
            ...
            _save_upload(samplesheet_file, samplesheet_path)
    else:
        counts_path = input_dir / counts_file.name
        metadata_path = input_dir / metadata_file.name
        _save_upload(counts_file, counts_path)
...
def _render_results_explorer(results_dir: Path, reports_dir: Path) -> None:
    ...
    de_path = results_dir / "differential_expression.csv"
...
def _render_gene_search_panel(
    ...
    with st.expander("Gene and expression profile", expanded=True):
        ...
        if active_gene:
            _render_gene_expression_profile(active_gene, norm_path, results_dir)
...
def _render_selected_gene_plots(
    ...
    with pca_col:
        _render_selected_gene_pca(matrix, metadata, px)
...
def _render_selected_gene_pca(matrix: pd.DataFrame, metadata: pd.DataFrame, px) -> None:
    import streamlit as st
    ...
    color_candidates = [col for col in pca_df.columns if col not in {"sample_id", "PC1", "PC2"}]
    ...
    fig = px.scatter(
        ...
        title=f"PCA of selected genes ({len(matrix)} geni)",
    )
    fig.update_layout(
        height=460,
...
def _render_selected_gene_heatmap(matrix: pd.DataFrame, go) -> None:
    ...
    fig.update_layout(
        title=f"Selected-gene heatmap sorted by logFC ({len(matrix)} geni)",
        height=460,
...
def _render_gene_expression_profile(
    ...
    if group_col:
        category_orders[group_col] = comparison_groups
